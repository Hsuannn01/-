...
# 將數據轉換為 numpy 陣列
for key in ['open', 'low', 'high', 'close', 'volume', 'amount']:
    KBar_dic[key] = np.array(list(KBar_dic[key].values()))
...
cycle_duration_option = st.selectbox("設定一根 K 棒的時間長度 (單位:日, 週, 月)", ['日', '周', '月'])

cycle_duration_map = {'日': 1440, '周': 10080, '月': 43200}  # 定義各單位的分鐘數
cycle_duration = cycle_duration_map[cycle_duration_option]
